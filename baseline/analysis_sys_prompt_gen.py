...
def process_batch(batch, tokenizer, model, rm_pipe, pipe_kwargs, device):
    ...
    sys_prompts = [system_prompt[6]+" " + p for p in prompts]
    ...
    formatted_responses = ["###human: " + prompt + " ###assistant: " + generated_text[len(prompt+ system_prompt[6]):] for prompt, generated_text in zip(prompts, generated_texts)]
